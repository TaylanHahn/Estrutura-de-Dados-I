...
// --- TODO (Melhorias) ---
// (para as melhorias 1 e 4, talvez seja útil incluir a <ctype.h>)
// #include <ctype.h> 
...
tContato lerContato() {
	...
	// --- TODO (Melhoria 1): validador de cpf ---
	// o que acontece se o usuário digitar "ABC" aqui?
	// ou se digitar um cpf com 5 dígitos?
	// a função 'lerContato' deveria validar a entrada do cpf.
	
	printf("digite o Nome: ");
	scanf(" %[^\n]", c.nome);

	// --- TODO (Melhoria 2): validar entrada vazia ---
	// e se o usuário só apertar "enter" no nome?
	// deveríamos checar se a string 'c.nome' está vazia (strlen == 0).
...
void consultarPorPosicao(tLista *lista, int pos) {
	...
	// --- TODO (Melhoria 6): otimização da busca ---
	// esta busca sempre começa do 'head'.
	// e se a 'pos' for 99 em uma lista de 100?
	// seria mais rápido começar pelo 'tail' e usar o 'ant'?
	// (dica: checar 'if (pos > lista->size / 2)' ...)
...
int removerPorPosicao(tLista *lista, int pos) {
	...
	// CASO 3: remoção do Meio
		...
		// --- TODO (Melhoria 6): otimização da busca ---
		// aqui também! esta busca para remoção também pode ser otimizada.
		// (dica: checar 'if (pos > lista->size / 2)' ...)
...
// --- TODO (Melhorias 3, 4, 5): stubs das novas funções ---
// (descomente os blocos abaixo para implementar as funcionalidades)
...
void buscarPorCPF(tLista *lista) {
	...
	// 1. pedir o cpf
	// 2. percorrer a lista (como no 'listarTodos')
	// 3. usar 'compararCPF' para achar
	// 4. imprimir se achar, ou "não encontrado" se não achar
	printf("\n(função 'buscarPorCPF' ainda não implementada)\n");
}
*/
...
void buscarPorNome(tLista *lista) {
	// TODO: implementar a lógica de busca por nome (parcial)
	// 1. pedir o nome (ou parte)
	// 2. percorrer a lista
	// 3. dica: usar strstr() para busca parcial (da <string.h>)
	// 4. dica 2: converter tudo para minúsculo (tolower() da <ctype.h>) para a busca
	printf("\n(função 'buscarPorNome' ainda não implementada)\n");
}
*/
...
void alterarContato(tLista *lista) {
	...
	// 1. buscar o contato (pode ser por cpf, reutilizando a busca 3)
	// 2. se achar, mostrar um menu para alterar (nome, email, telefone)
	// 3. salvar a alteração no nó encontrado
	printf("\n(função 'alterarContato' ainda não implementada)\n");
}
*/
