...
tContato lerContato();
	...
	// AGORA INCLUI VALIDAÇÃO DE CPF E ENTRADA VAZIA
...
int removerPorPosicao(tLista *lista, int pos);
	...
	// AGORA OTIMIZADO PARA BUSCAR DO FIM (TAIL)
...
// add por Taylan: ----------------- NOVAS FUNÇÕES -----------------
// Protótipos das novas funções solicitadas

void consultarPorCPF(tLista *lista);
	// (Req 3) Pede um CPF ao usuário e busca o contato correspondente

void buscarPorNome(tLista *lista);
	// (Req 4) Pede um nome (ou parte) e busca contatos (case-insensitive)

void alterarContato(tLista *lista);
	// (Req 5) Pede um CPF, encontra o contato e permite alterar nome, email e telefone
// add por Taylan: ----------------- FIM DAS NOVAS FUNÇÕES -----------------