...
⚙️ Funcionalidades mínimas (e adicionais)
...
+ (add Taylan) Validar CPF (11 dígitos numéricos)
+ (add Taylan) Validar entradas vazias
+ (add Taylan) Buscar por CPF
+ (add Taylan) Buscar por Nome (parcial)
+ (add Taylan) Alterar contato
+ (add Taylan) Otimizar busca por posição e remoção (usando tail)
...
int main() {
	...
	// add por Taylan: Novas opções de menu
	printf("4 - Consultar contato por CPF\n");
	printf("5 - Buscar contato por Nome\n");
	printf("6 - Alterar contato (por CPF)\n");
	printf("7 - Listar todos os contatos\n"); // (era 4)
	printf("0 - Sair\n");
	printf("-------------------------------------");
	printf("\nOpcao: ");
	...
	scanf("%d", &escolha);
		...
		switch (escolha) {
			...
			case 1:
				...
				// chama a função auxiliar para ler os dados (agora com validação)
			...
			// consultar contato por posição
			case 3: 
				printf("\nMENU > 3. Consultar contato por POSICAO\n");
				if(estaVazia(&lista)) {
			...
			// add por Taylan: (Req 3) Novo case para consultar por CPF
			case 4:
				printf("\nMENU > 4. Consultar contato por CPF\n");
				consultarPorCPF(&lista); // a função já cuida de tudo
				break;

			// add por Taylan: (Req 4) Novo case para buscar por Nome
			case 5:
				printf("\nMENU > 5. Buscar contato por NOME\n");
				buscarPorNome(&lista); // a função já cuida de tudo
				break;
			
			// add por Taylan: (Req 5) Novo case para alterar contato
			case 6:
				printf("\nMENU > 6. Alterar contato\n");
				alterarContato(&lista); // a função já cuida de tudo
				break;
			
			// listar todos (agora é case 7)
			case 7:
				printf("\nMENU > 7. Listar todos\n");
		...
		// add por Taylan: Lógica de listagem atualizada
		// Lista tudo automaticamente após Inserir (1), Remover (2), Alterar (6)
		// e, claro, ao Listar (7).
		// Não lista após Consultas (3, 4, 5), Sair (0) ou opção inválida.
		if (escolha != 3 && escolha != 4 && escolha != 5 && escolha != 0 && escolha > 0 && escolha <= 7) {
			 listarTodos(&lista);
		}
